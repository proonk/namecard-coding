...
def draw_marks_and_grid(
  ...
  line_dist_x = 5.0 * mm  # 左右两侧横向角线距离成品边缘留白 0.5 cm
  paper_margin = 3.0 * mm  # 距离 A3 纸张顶底边缘留白 0.3 cm
...
def make_imposition_pdf(
    paper_choice, use_bleed, front_path, back_path, output_pdf
):
  mm = 72 / 25.4

  card_w = 90.0 * mm  # 成品宽度：9.0 cm
  card_h = 54.0 * mm  # 成品高度：5.4 cm
  ...
  if paper_choice == "1":
    ...
    paper_name = f"A3 横向 ({'带1.5mm出血' if use_bleed else '无出血'})"
  else:
    ...
    x_positions = [start_x_left, start_x_left + card_w + gutter_x]
    paper_name = f"A4 竖向 ({'带1.5mm出血' if use_bleed else '无出血'})"
...
if __name__ == "__main__":
  ...
  bleed_input = (
      input("是否添加 1.5mm 出血(Bleed)？(Y/N，默认 Y): ").strip().upper()
  )
  use_bleed = bleed_input != "N"
  ...
  make_imposition_pdf(choice, use_bleed, front_img, back_img, out_file)